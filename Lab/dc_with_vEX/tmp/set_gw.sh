#!/bin/bash
echo "
127.0.0.1 localhost
::1 ip6-localhost ip6-loopback
172.16.10.2 apstra
172.16.10.3 ztp
172.16.12.10 svr11
127.0.1.1 gw
" | sudo tee /etc/hosts
echo "
default-lease-time 600;
max-lease-time 7200;
ddns-update-style none;
option space ZTP;
option ZTP.server-image code 0 = text;
option ZTP.server-file code 1 = text;
option ZTP.image-file-type code 2 = text;
option ZTP.transfer-mode code 3 = text;
option ZTP.symlink-server-image code 4 = text;
option ZTP.http-port code 5 = text;
option ZTP-encapsulation code 43 = encapsulate ZTP;
subnet 172.16.12.0 netmask 255.255.255.0 {
   range 172.16.12.5 172.16.12.20;
   option routers 172.16.12.1;
   option domain-name-servers 66.129.233.81;
}
host apstra {
   hardware ethernet 56:01:08:00:64:31;
   fixed-address 172.16.10.2;
}
host ztp {
   hardware ethernet 56:01:08:00:4e:aa;
   fixed-address 172.16.10.3;
}
host svr11 {
   hardware ethernet 56:01:08:00:55:63;
   fixed-address 172.16.12.10;
}
" | sudo tee /etc/dhcp/dhcpd.conf
rm -f ~/.ssh/*
echo "ssh-rsa  irzan@irzan-mbp" | tee .ssh/authorized_keys
echo "-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----" | tee .ssh/id_rsa
chmod og-rwx .ssh/id_rsa
echo "
network:
  ethernets:
    eth1:
       addresses: [ 172.16.10.1/24 ]
    eth2:
       addresses: [ 172.16.12.1/24 ]
" | sudo tee /etc/netplan/02_net.yaml
sudo rm /etc/resolv.conf
echo "
nameserver 66.129.233.81
nameserver 66.129.233.82
" | sudo tee /etc/resolv.conf
echo "Host *
   StrictHostKeyChecking no
" | tee ~/.ssh/config

echo '#!/bin/bash
websockify -D --web=/usr/share/novnc/ 6081 sv8-pod-j31.englab.juniper.net:5902
websockify -D --web=/usr/share/novnc/ 6082 sv8-pod-j29.englab.juniper.net:5905
websockify -D --web=/usr/share/novnc/ 6083 sv8-pod-j24.englab.juniper.net:5915
websockify -D --web=/usr/share/novnc/ 6084 sv8-pod-j20.englab.juniper.net:5902
websockify -D --web=/usr/share/novnc/ 6085 sv8-pod-j01.englab.juniper.net:5904
websockify -D --web=/usr/share/novnc/ 6086 sv8-pod-j19.englab.juniper.net:5906
websockify -D --web=/usr/share/novnc/ 6087 sv8-pod-j04.englab.juniper.net:5905
websockify -D --web=/usr/share/novnc/ 6088 sv8-pod-j40.englab.juniper.net:5904
websockify -D --web=/usr/share/novnc/ 6089 sv8-pod-g23.englab.juniper.net:5958
websockify -D --web=/usr/share/novnc/ 6090 sv8-pod-g03.englab.juniper.net:5902
websockify -D --web=/usr/share/novnc/ 6091 sv8-pod-j40.englab.juniper.net:5905
websockify -D --web=/usr/share/novnc/ 6092 sv8-pod-g23.englab.juniper.net:5959
websockify -D --web=/usr/share/novnc/ 6093 sv8-pod-j33.englab.juniper.net:5902
websockify -D --web=/usr/share/novnc/ 6094 sv8-pod-j24.englab.juniper.net:5914
websockify -D --web=/usr/share/novnc/ 6095 sv8-pod-j23.englab.juniper.net:5912
' | sudo tee  /usr/local/bin/startup.sh
sudo chmod +x /usr/local/bin/startup.sh
echo "#!/bin/bash
echo "-------------------------"
echo "URL access to VNC: "
echo "console apstra : http://172.16.10.1:6081/vnc.html"
echo "console ztp : http://172.16.10.1:6082/vnc.html"
echo "console svr1 : http://172.16.10.1:6083/vnc.html"
echo "console svr2 : http://172.16.10.1:6084/vnc.html"
echo "console svr3 : http://172.16.10.1:6085/vnc.html"
echo "console svr4 : http://172.16.10.1:6086/vnc.html"
echo "console svr5 : http://172.16.10.1:6087/vnc.html"
echo "console svr6 : http://172.16.10.1:6088/vnc.html"
echo "console svr7 : http://172.16.10.1:6089/vnc.html"
echo "console svr8 : http://172.16.10.1:6090/vnc.html"
echo "console svr9 : http://172.16.10.1:6091/vnc.html"
echo "console svr10 : http://172.16.10.1:6092/vnc.html"
echo "console lxc1 : http://172.16.10.1:6093/vnc.html"
echo "console lxc2 : http://172.16.10.1:6094/vnc.html"
echo "console svr11 : http://172.16.10.1:6095/vnc.html"
echo "-------------------------"" | sudo tee /etc/update-motd.d/99-update
sudo chmod +x /etc/update-motd.d/99-update
echo "/usr/local/bin/startup.sh" | sudo tee -a /etc/rc.local

sudo sed -i -e "s/#DNS=/DNS=66.129.233.81/" /etc/systemd/resolved.conf
sudo sed -i -e "s/#FallbackDNS=/FallbackDNS=66.129.233.82/" /etc/systemd/resolved.conf

sleep 2
sudo netplan apply
sudo systemctl restart rc-local.service
sudo systemctl restart isc-dhcp-server
sudo systemctl restart systemd-resolved.service
