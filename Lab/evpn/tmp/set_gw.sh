#!/bin/bash
echo "
127.0.0.1 localhost
::1 ip6-localhost ip6-loopback
172.16.10.201 client1
172.16.10.202 client2
172.16.10.203 client3
172.16.10.204 br1
127.0.1.1 gw
" | sudo tee /etc/hosts
echo "
default-lease-time 600;
max-lease-time 7200;
ddns-update-style none;
option space ZTP;
option ZTP.server-image code 0 = text;
option ZTP.server-file code 1 = text;
option ZTP.image-file-type code 2 = text;
option ZTP.transfer-mode code 3 = text;
option ZTP.symlink-server-image code 4 = text;
option ZTP.http-port code 5 = text;
option ZTP-encapsulation code 43 = encapsulate ZTP;
subnet 172.16.10.0 netmask 255.255.255.0 {
   range 172.16.10.200 172.16.10.220;
   option routers 172.16.10.1;
   option domain-name-servers 66.129.233.81;
}
host client1 {
   hardware ethernet 56:04:19:00:2b:86;
   fixed-address 172.16.10.201;
}
host client2 {
   hardware ethernet 56:04:19:00:6c:fb;
   fixed-address 172.16.10.202;
}
host client3 {
   hardware ethernet 56:04:19:00:a3:11;
   fixed-address 172.16.10.203;
}
host br1 {
   hardware ethernet 56:04:19:00:76:df;
   fixed-address 172.16.10.204;
}
" | sudo tee /etc/dhcp/dhcpd.conf
rm -f ~/.ssh/*
echo "ssh-rsa  irzan@irzan-mbp" | tee .ssh/authorized_keys
echo "-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----" | tee .ssh/id_rsa
chmod og-rwx .ssh/id_rsa
echo "
network:
  ethernets:
    eth1:
       addresses: [ 172.16.10.1/24 ]
" | sudo tee /etc/netplan/02_net.yaml
sudo rm /etc/resolv.conf
echo "
nameserver 66.129.233.81
nameserver 66.129.233.82
" | sudo tee /etc/resolv.conf
echo "Host *
   StrictHostKeyChecking no
" | tee ~/.ssh/config

echo '#!/bin/bash
websockify -D --web=/usr/share/novnc/ 6081 q-pod-73e.englab.juniper.net:5926
websockify -D --web=/usr/share/novnc/ 6082 q-pod-73c.englab.juniper.net:5968
websockify -D --web=/usr/share/novnc/ 6083 q-pod-75l.englab.juniper.net:5905
websockify -D --web=/usr/share/novnc/ 6084 q-pod-73c.englab.juniper.net:5967
' | sudo tee  /usr/local/bin/startup.sh
sudo chmod +x /usr/local/bin/startup.sh
echo "#!/bin/bash
echo "-------------------------"
echo "URL access to VNC: "
echo "console client1 : http://172.16.10.1:6081/vnc.html"
echo "console client2 : http://172.16.10.1:6082/vnc.html"
echo "console client3 : http://172.16.10.1:6083/vnc.html"
echo "console br1 : http://172.16.10.1:6084/vnc.html"
echo "-------------------------"" | sudo tee /etc/update-motd.d/99-update
sudo chmod +x /etc/update-motd.d/99-update
echo "/usr/local/bin/startup.sh" | sudo tee -a /etc/rc.local

sudo sed -i -e "s/#DNS=/DNS=66.129.233.81/" /etc/systemd/resolved.conf
sudo sed -i -e "s/#FallbackDNS=/FallbackDNS=66.129.233.82/" /etc/systemd/resolved.conf

sleep 2
sudo netplan apply
sudo systemctl restart rc-local.service
sudo systemctl restart isc-dhcp-server
sudo systemctl restart systemd-resolved.service
